...
def get_traffic_info_from_dict(traffic_info):
    try:
        ...
        geometry = traffic_feature.get('geometry', {})

        # 기본 정보
        link_id = traffic_properties.get('id', 'null')
        link_name = traffic_properties.get('name', 'null')
        description = traffic_properties.get('description', 'null') or 'null'
        congestion = int(traffic_properties.get('congestion', -1))
        link_direction = int(traffic_properties.get('direction', -1)) if traffic_properties.get('direction') != 'null' else 'null'
        link_roadType = traffic_properties.get('roadType', 'null')
        startNodeName = traffic_properties.get('startNodeName', 'null') or 'null'
        endNodeName = traffic_properties.get('endNodeName', 'null') or 'null'
        link_length = float(traffic_properties.get('distance', 0.0))
        link_pass_time = float(traffic_properties.get('time', 0.0))
        link_speed = float(traffic_properties.get('speed', 0.0))
        traffic_updateTime = traffic_properties.get('updateTime', 'null')

        # 추가 정보
        geometry_type = geometry.get('type', 'null')  # 'LineString' or 'Point'
        coordinates = geometry.get('coordinates', [])  # 실제 좌표들 (List of [lon, lat])
        coordinates_str = str(coordinates)  # 문자열로 저장

        line_index = traffic_properties.get('lineIndex', -1)
        index = traffic_properties.get('index', -1)

        is_accident_node = traffic_properties.get('isAccidentNode', 'N')
        accident_code = traffic_properties.get('accidentUpperCode', '')
        accident_name = traffic_properties.get('accidentUpperName', '')

        traffic_data = [
            link_id, link_name, description, congestion, traffic_updateTime,
            link_direction, link_roadType, startNodeName, endNodeName,
            link_length, link_pass_time, link_speed,
            geometry_type, coordinates_str,
            line_index, index,
            is_accident_node, accident_code, accident_name
        ]
    except Exception as e:
        print(f"[WARNING] Traffic parsing failed: {e}")
        traffic_data = None
...
def receive_GNSS(d_name, save_flag, SAVE_PATH, config=None, print_status=True, receive_trf_info=False, stop_event=None):
    ...
    if receive_trf_info:
        ...
        traffic_cols = [
            'LinkID', 'LinkName', 'Description', 'Congestion', 'TrafficUpdateTime',
            'LinkDirection', 'LinkRoadType', 'StartNodeName', 'EndNodeName',
            'LinkLength', 'LinkPassTime', 'LinkSpeed',
            'GeometryType', 'Coordinates',  			      # 도로 선형 정보 추가 gawon
            'LineIndex', 'Index',           			      # 링크 순서 정보 추가 gawon
            'IsAccidentNode', 'AccidentCode', 'AccidentName'  # 돌발 정보 정보 추가 gawon
        ]
